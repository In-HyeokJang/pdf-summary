...
/**
 * Google Gemini API를 호출하는 LLM 서비스 구현체.
 *
...
class GeminiService(
    ...
    private val client: WebClient by lazy {
        webClientConfig.buildWebClient("https://generativelanguage.googleapis.com")
    }
    ...
    private data class GeminiRequest(
        ...
        data class GenerationConfig(val maxOutputTokens: Int, val temperature: Double = 0.3)
    }
...
$text
        ...
        log.info("[Gemini TIMING] 요약 요청 ({}자)", text.length)
    ...
    private fun callAsync(system: String, user: String, maxTokens: Int): Mono<String> {
        ...
        val request = GeminiRequest(
            system_instruction = GeminiRequest.SystemInstruction(listOf(GeminiRequest.Part(system))),
            contents = listOf(GeminiRequest.Content(listOf(GeminiRequest.Part(user)))),
